#!/usr/bin/env bash
# Deploy authorized_admins.json and update reconcile script to suppress false CRITICAL alerts
set -euo pipefail

echo "=== Deploy: Authorized GW Admins ==="
echo ""

# ── 1. Write authorized_admins.json ────────────────────────────────────────
echo "Step 1: Write /opt/monitoring/data/authorized_admins.json"
cat > /opt/monitoring/data/authorized_admins.json << 'JSON'
[
  {
    "email": "brian.monte@yokly.gives",
    "name": "Brian Gregory Monte",
    "role": "IT Admin / Owner"
  },
  {
    "email": "csednie.regasa@yokly.gives",
    "name": "Csednie Regasa",
    "role": "Junior Developer"
  },
  {
    "email": "josh@yokly.gives",
    "name": "Majesty Josh Carsido",
    "role": "Business Ops Manager"
  },
  {
    "email": "markangel@yokly.gives",
    "name": "Mark Angel Quiatchon",
    "role": "IT Lead"
  },
  {
...
echo "  Written (5 authorized admins)"

# ── 2. Rewrite the reconcile script ────────────────────────────────────────
echo ""
echo "Step 2: Update /opt/monitoring/bin/employee-gworkspace-reconcile.py"
...
try:
    creds = service_account.Credentials.from_service_account_file(
        SA_KEY, scopes=["https://www.googleapis.com/auth/admin.directory.user.readonly"],
        subject=ADMIN
    )
except Exception as e:
...
#    Split into: truly unauthorized vs known-authorized-but-not-in-roster
admin_unregistered = []
...
orphaned_unauthorized_admins = [o for o in orphaned if o["is_admin"] and not o["authorized"]]
...
emit_prom("employee_reconcile_admin_unregistered", len(unauthorized_admins),
          help_text="GW admin accounts not in roster and not authorized")
...
for o in orphaned:
    ...
    auth_str     = "true" if o["authorized"] else "false"
    lbl = f'email="{o["email"]}",name="{o["name"]}",is_admin="{is_admin_str}",authorized="{auth_str}"'
    prom.append(f"employee_reconcile_orphan_info{{{lbl}}} 1")

# Authorized admins metric (visible in Grafana)
...
emit_wazuh("EmployeeReconcileSummary", "info",
    ...
    admin_unregistered_count=len(unauthorized_admins),
)
...
for o in orphaned:
    if o["is_admin"]:
        if o["authorized"]:
            # Known authorized admin — orphaned from roster but not a threat
            emit_wazuh("EmployeeOrphanedGWAccount", "info",
                f"Authorized GW admin {o['email']} is not in employee roster — verify if intentional",
                email=o["email"],
                full_name=o["name"],
                is_admin="true",
                authorized="true",
...
# Truly unauthorized admin accounts (not in roster AND not authorized)
...
# ── 3. Run reconciliation now ───────────────────────────────────────────────
echo ""
echo "Step 3: Run reconciliation"
...
echo ""
echo "Step 4: Verify Prometheus metrics"
grep "authorized_admin" /opt/monitoring/textfile_collector/employee_reconcile.prom

echo ""
echo "=== Done ==="
echo "authorized_admins.json: /opt/monitoring/data/authorized_admins.json"
echo "To add/remove admins, edit that file and re-run reconciliation."
