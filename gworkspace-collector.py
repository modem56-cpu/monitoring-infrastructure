...
try:
    ...
    prom_lines.append("# HELP gworkspace_drive_usage_bytes Per-user total account storage (Drive + Gmail + Photos); source: accounts:used_quota_in_mb")
    prom_lines.append("# TYPE gworkspace_drive_usage_bytes gauge")
    prom_lines.append("# HELP gworkspace_drive_only_bytes Per-user My Drive-only storage (excl. Gmail and Photos); source: accounts:drive_used_quota_in_mb")
    ...
    report_date_used = None  # tracks which API date returned data (for freshness metrics)
    for u in active_users:
        ...
        for delta in range(2, 9):
            ...
            try:
                ...
                if report_date_used is None and resp.get("usageReports"):
                    report_date_used = d  # capture first date that returned real data
    ...
    if report_date_used is not None:
        _report_epoch = int(datetime.datetime.strptime(report_date_used, "%Y-%m-%d")
                            .replace(tzinfo=datetime.timezone.utc).timestamp())
        _lag_days = (date.today() - date.fromisoformat(report_date_used)).days
        prom_lines.append("# HELP gworkspace_report_date_unixtime Date of Google Workspace Reports API data (Unix timestamp, midnight UTC)")
        prom_lines.append("# TYPE gworkspace_report_date_unixtime gauge")
        emit_prom("gworkspace_report_date_unixtime", _report_epoch)
        prom_lines.append("# HELP gworkspace_report_lag_days Days between Reports API data date and today (expected 2–8 days due to Google API lag)")
        prom_lines.append("# TYPE gworkspace_report_lag_days gauge")
        emit_prom("gworkspace_report_lag_days", _lag_days)
...
APPROVED_DRIVES_FILE = Path(os.environ.get(
    ...
    "/opt/monitoring/data/approved_external_shared_drives.json"
))
...
try:
    ...
    prom_lines.append("# HELP gworkspace_shared_drive_size_bytes Shared drive storage (full enumeration via quotaBytesUsed)")
...
try:
    ...
    prom_lines.append("# HELP gworkspace_org_drive_bytes Org Drive-only storage (My Drive; excl. Gmail and Photos); source: accounts:drive_used_quota_in_mb")
    ...
    prom_lines.append("# HELP gworkspace_org_shared_drive_bytes Org shared drive storage total (full enumeration via quotaBytesUsed)")
...
# Write outputs
# ============================================================
prom_lines.append("# HELP gworkspace_collector_last_success_unixtime Unix timestamp of last successful collection run")
prom_lines.append("# TYPE gworkspace_collector_last_success_unixtime gauge")
emit_prom("gworkspace_collector_last_success_unixtime", int(time.time()))
