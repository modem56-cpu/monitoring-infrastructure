#!/usr/bin/env bash
# Implement all remaining P3/P4 items requiring root:
#   1. Deploy authorized_admins.json + updated reconcile script
#   2. Persist iptables rules (iptables-persistent)
#   3. logrotate for employee-gworkspace-wazuh.log
#   4. Systemd timer — daily Google Sheet → employees.json sync
#   5. Add ClickHouse disk + Unraid array Prometheus alert rules
#   6. Reload Prometheus
set -euo pipefail

echo "=== P3/P4 Root Fixes ==="
echo ""

# ── 1. Authorized admins + updated reconcile script ────────────────────────
echo "Step 1: Authorized admins"
cp /tmp/authorized_admins.json /opt/monitoring/data/authorized_admins.json
chmod 644 /opt/monitoring/data/authorized_admins.json
echo "  /opt/monitoring/data/authorized_admins.json written"
...
PYEOF
chmod 755 /opt/monitoring/bin/employee-gworkspace-reconcile.py
echo "  Reconcile script updated"

# Run reconcile now to confirm
...
echo "Step 2: Persist iptables rules"
...
# ── 3. Logrotate for employee-gworkspace-wazuh.log ────────────────────────
echo ""
echo "Step 3: Logrotate"
cat > /etc/logrotate.d/employee-reconcile << 'LOGROTATE'
...
LOGROTATE
cat > /etc/logrotate.d/prometheus-wazuh << 'LOGROTATE'
/var/log/prometheus-wazuh.log {
    daily
    rotate 30
    compress
    delaycompress
...
LOGROTATE
logrotate --debug /etc/logrotate.d/employee-reconcile 2>&1 | head -5
echo "  logrotate configs written"

# ── 4. Systemd timer — daily Google Sheet sync ────────────────────────────
echo ""
echo "Step 4: Systemd timer for daily Sheet → employees.json sync"
cat > /etc/systemd/system/employees-sheet-sync.service << 'SVC'
[Unit]
Description=Sync active employees from Google Sheet to employees.json
...
Type=oneshot
User=root
Environment="OUT_FILE=/opt/monitoring/data/employees.json"
Environment="SA_KEY=/opt/monitoring/gam-project-gf5mq-97886701cbdd.json"
Environment="ADMIN_EMAIL=brian.monte@yokly.gives"
ExecStart=/usr/bin/python3 /opt/monitoring/sync-employees-from-sheet.py
StandardOutput=journal
StandardError=journal
SVC

cat > /etc/systemd/system/employees-sheet-sync.timer << 'TIMER'
[Unit]
Description=Daily Google Sheet → employees.json sync (08:00 local)
Requires=employees-sheet-sync.service
...
systemctl enable --now employees-sheet-sync.timer
systemctl is-active employees-sheet-sync.timer && echo "  Timer active: employees-sheet-sync.timer"

# ── 5. Prometheus alert rules — ClickHouse disk + Unraid array ────────────
echo ""
echo "Step 5: Prometheus rules — disk growth alerts"

# Append to infrastructure.rules.yml
cat >> /opt/monitoring/rules/infrastructure.rules.yml << 'RULES'

  # ── Unraid Array Capacity ──────────────────────────────────────────────
  - alert: UnraidArrayAlmostFull
    expr: tower_unraid_array_used_percent > 85
    for: 5m
    labels:
      severity: warning
    annotations:
      summary: "Unraid array {{ $labels.target }} at {{ $value | humanize }}% capacity"
      description: "Array usage exceeds 85%. Free up space or expand array before it fills."

  - alert: UnraidArrayCritical
    expr: tower_unraid_array_used_percent > 95
    for: 2m
    labels:
      severity: critical
    annotations:
      summary: "Unraid array {{ $labels.target }} CRITICAL — {{ $value | humanize }}% full"
      description: "Array over 95% — writes may fail. Immediate action required."

  - alert: UnraidCacheAlmostFull
    expr: tower_unraid_cache_used_percent > 90
    for: 5m
    labels:
      severity: warning
    annotations:
      summary: "Unraid cache pool at {{ $value | humanize }}%"
      description: "Cache pool is nearly full — mover may need to run or cache needs expansion."

  # ── Host Disk (catches ClickHouse data dir on wazuh-server) ───────────
  - alert: HostDiskAlmostFull
    expr: |
      (1 - node_filesystem_avail_bytes{fstype!~"tmpfs|overlay|devtmpfs",mountpoint="/"} /
           node_filesystem_size_bytes{fstype!~"tmpfs|overlay|devtmpfs",mountpoint="/"}) * 100 > 80
    for: 5m
    labels:
      severity: warning
    annotations:
      summary: "Disk on {{ $labels.instance }} at {{ $value | humanize }}%"
      description: "Root filesystem over 80% — check ClickHouse, Wazuh indexes, logs."

  - alert: HostDiskCritical
    expr: |
      (1 - node_filesystem_avail_bytes{fstype!~"tmpfs|overlay|devtmpfs",mountpoint="/"} /
           node_filesystem_size_bytes{fstype!~"tmpfs|overlay|devtmpfs",mountpoint="/"}) * 100 > 92
    for: 2m
    labels:
      severity: critical
    annotations:
      summary: "Disk CRITICAL on {{ $labels.instance }} — {{ $value | humanize }}% used"
      description: "Root filesystem over 92%. Services may start failing. Check ClickHouse trace_log and Wazuh indexes."
RULES

echo "  Rules appended to infrastructure.rules.yml"

# ── 6. Reload Prometheus ───────────────────────────────────────────────────
echo ""
echo "Step 6: Validate and reload Prometheus"
docker exec prometheus promtool check rules /etc/prometheus/rules/infrastructure.rules.yml && \
  echo "  Rules valid" || { echo "  ERROR in rules"; exit 1; }
...
echo "  Alertmanager email — add Gmail app password to /opt/monitoring/alertmanager.yml"
echo "  Line: smtp_auth_password: 'REPLACE_WITH_GMAIL_APP_PASSWORD'"
echo "  Then: docker restart alertmanager"
